...
def arena_viva():
    ...
    with col_jogo:
        ...
        with c_img:
            # Busca a imagem baseada nos erros globais
        ...
        # --- LÓGICA DE FIM DE JOGO E BALÕES ---
        if vitoria and erros_atuais < 6:
            if contagem == 0:
                st.success("🎉 VITÓRIA FINAL DA EQUIPE!")
                # st.balloons() # Desativado conforme solicitado
            else:
            ...
            if contagem == 0:
                ...
                # Dispara balões apenas uma vez no fim da arena
                if "baloes_disparados" not in st.session_state:
                    st.balloons()
                    st.session_state.baloes_disparados = True
        else:
            # Interface do Teclado (só aparece se o jogo estiver rolando)
    ...
    with col_rank:
        ...
        st.divider()
        # Botão de Reset acessível a todos
        if st.button("🔄 REINICIAR ARENA", use_container_width=True):
            # Limpa o estado local de balões antes de resetar o banco
