...
export default function ArenaDaForca() {
  ...
  useEffect(() => {
    // Busca inicial do estado da arena
    ...
    // Inscrição nos canais em tempo real (Substitui o run_every=1 do Streamlit)
  ...
  // --- LÓGICA DO CRONÔMETRO SERVERLESS ---
  useEffect(() => {
    ...
    const intervalo = setInterval(() => {
      if (jogo.forca_timestamp_inicio) {
        ...
        // Se o tempo esgotou e quem devia jogar era o jogador da vez
  ...
  const calcularProximoTurno = async (atual: string) => {
    ...
    const lista = data.map(r => r.jogador);
  ...
  const registrarJogada = async (letra: string) => {
    ...
    if (vitoria) {
      await supabase.from("forca_disputa_ranking").update({ pontos: ptsAtuais + 5 + 10 }).eq("jogador", jogador);
    }
  ...
  // --- FUNÇÕES DO MESTRE ---
  const avancarPergunta = async (lista: any[] = filaPerguntas) => {
    if (lista.length === 0) return;
    const proxima = lista[0];
    const novaFila = lista.slice(1);
      ...
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-4">
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-xl max-w-md w-full shadow-2xl">
          ...
          <div className="flex gap-4 mb-6 bg-slate-950 p-1 rounded-lg border border-slate-800">
            <button 
              className={`flex-1 py-2 rounded-md font-medium transition ${perfil === "Jogador" ? "bg-blue-600 text-white" : "text-slate-400"}`}
              onClick={() => setPerfil("Jogador")}
            >Jogador</button>
            <button 
              className={`flex-1 py-2 rounded-md font-medium transition ${perfil === "Mestre" ? "bg-blue-600 text-white" : "text-slate-400"}`}
              onClick={() => setPerfil("Mestre")}
            >Mestre (Admin)</button>
          </div>
          ...
          <button onClick={realizarLogin} className="w-full mt-6 bg-blue-600 hover:bg-blue-500 p-3 rounded-lg font-bold transition shadow-lg shadow-blue-950">
            ENTRAR NA ARENA
          </button>
        </div>
      </div>
  ...
  if (jogador !== "TREINAMENTOWLI") {
      ...
      <div className="min-h-screen bg-slate-950 text-white p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="md:col-span-3 bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col justify-between min-h-[400px]">
            <div>
              ...
              <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 text-lg mb-6 text-slate-200">
                ❓ {jogo?.pergunta || "Aguardando o mestre iniciar..."}
              </div>
            ...
            {!vitoria && erros < 6 ? (
                ...
                {"ABCDEFGHIJKLMNOPQRSTUVWXYZ-".split("").map(letra => {
                  ...
                  return (
                    <button 
                      key={letra} 
                      disabled={bloqueado}
                      onClick={() => registrarJogada(letra)}
                      className={`p-3 rounded-lg font-bold text-center transition ${jaFoi ? "bg-slate-800 text-slate-600 cursor-not-allowed" : bloqueado ? "bg-slate-900 text-slate-700 cursor-not-allowed border border-slate-950" : "bg-blue-600 hover:bg-blue-500 text-white"}`}
                    >
                      {letra}
                    </button>
                  );
              ...
              <div className="text-center p-4 bg-slate-950 rounded-lg border border-slate-800 text-amber-400 font-bold">
                {vitoria ? "🎉 Palavra Descoberta! Aguarde a próxima rodada." : "💀 Fim de Jogo! A equipe errou 6 vezes."}
              </div>
            )}
          </div>
        ...
        <div className="mt-8 bg-slate-900 border border-slate-800 rounded-xl p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            {ranking.filter(r => r.jogador !== "TREINAMENTOWLI").slice(0, 10).map((r, i) => (
    ...
    // --- PAINEL COMPLETO DO MESTRE DE JOGO (ADMIN) ---
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6">
      <header className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
        <h1 className="text-2xl font-bold text-amber-400">⚔️ Painel do Mestre - Arena da Forca</h1>
        <div className="bg-slate-900 px-4 py-2 rounded-lg border border-slate-800 text-sm font-mono">
          Chave Atual: <span className="text-blue-400 font-bold tracking-wider">{jogo?.forca_senha_acesso || "----"}</span>
        </div>
      </header>
      ...
      <div className="flex gap-2 border-b border-slate-800 mb-6">
        {["🎮 Arena", "👥 Participantes", "🔄 Configurações"].map((aba, index) => (
          <button 
            key={aba} 
            className={`px-4 py-2 font-medium transition border-b-2 -mb-[2px] ${abaAtiva === index ? "border-blue-500 text-blue-400" : "border-transparent text-slate-400 hover:text-slate-200"}`}
      ...
      {abaAtiva === 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-3 bg-slate-900 border border-slate-800 rounded-xl p-6">
            ...
            <div className="bg-slate-950 p-4 rounded-lg border border-slate-800 text-center font-mono text-2xl font-bold tracking-widest text-red-400">
              {jogo?.palavra}
            </div>
            <button onClick={() => avancarPergunta()} className="w-full mt-4 bg-emerald-600 hover:bg-emerald-500 p-3 rounded-lg font-bold transition">
              🚀 LANÇAR PRÓXIMA PERGUNTA ({filaPerguntas.length} na fila)
            </button>
          </div>
      ...
      {abaAtiva === 1 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-2xl">
          <div className="flex justify-between items-center mb-6">
            ...
            }} className="bg-red-600 hover:bg-red-500 px-4 py-2 rounded-lg font-medium text-sm transition">
              🗑️ EXPULSAR TODOS
            </button>
          </div>
          <div className="space-y-2">
            {ranking.filter(r => r.jogador !== "TREINAMENTOWLI").map(r => (
              <div key={r.jogador} className="flex justify-between items-center bg-slate-950 p-3 rounded-lg border border-slate-800">
                ...
                }} className="bg-red-950 hover:bg-red-900 text-red-400 border border-red-900/50 px-3 py-1 rounded-md text-xs font-medium transition">
                  EXPULSAR
                </button>
              </div>
            ))}
      ...
      {abaAtiva === 2 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 max-w-xl space-y-6">
          ...
          <div className="border-t border-slate-800 pt-4">
            ...
            <div className="flex gap-4">
              {["LIVRE", "TURNOS"].map(modo => (
                <button 
                  key={modo}
                  onClick={async () => await supabase.from("forca_disputa_arena").update({ forca_modo_jogo: modo, forca_proximo_turno: "" }).eq("id", 1)}
                  className={`flex-1 p-3 rounded-lg font-bold border transition ${jogo?.forca_modo_jogo === modo ? "bg-blue-600 border-blue-500 text-white" : "bg-slate-950 border-slate-800 text-slate-400"}`}
                >
                  {modo}
                </button>
              ))}
          ...
          <div className="border-t border-slate-800 pt-4">
            <button onClick={reiniciarArena} className="w-full bg-slate-950 hover:bg-slate-900 border border-red-900 text-red-500 p-3 rounded-lg font-bold transition">
              🔄 REINICIAR ARENA COMPLETA
            </button>
          </div>
        </div>
      )}
